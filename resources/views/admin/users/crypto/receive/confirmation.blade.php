...
@section('title', 'Crypto Receive Confirm')
                    ...
                    <div class="col-md-7">
                        ...
                        <div class="panel panel-default">
                            <div class="panel-body">
                                ...
                                <div class="row">
                                    <div class="col-md-6 pull-left">Receive Amount</div>
                        ...
                        <div style="margin-left: 0 auto">
                            <div style="float: left;">
                                <a href="#" class="admin-user-crypto-receive-confirm-back-link">
                                    <button class="btn button-secondary admin-user-crypto-receive-confirm-back-btn"><strong><i class="fa fa-angle-left"></i>&nbsp;&nbsp;Back</strong></button>
                                </a>
                            </div>
                            <div style="float: right;">
                                <form action="{{ url('admin/users/deposit/crypto/receive/success') }}" style="display: block;" method="POST" accept-charset="UTF-8" id="admin-user-crypto-receive-confirm" novalidate="novalidate">
                                    <input value="{{csrf_token()}}" name="_token" id="token" type="hidden">
                                    <input type="hidden" name="user_id" value="{{ $cryptoTrx['user_id'] }}">

                                    <button type="submit" class="btn button-secondary" id="admin-user-crypto-receive-confirm-btn">
...
<script type="text/javascript">
    ...
    function cryptoReceiveConfirmBack()
    {
        ...
        window.location.replace(SITE_URL+`/admin/users/crypto/receive/${'{{ $user_id }}'}`);
    }
    ...
    $('#admin-user-crypto-receive-confirm').validate({
        submitHandler: function(form)
        {
            ...
            $(".fa-spin").show();
            var pretext=$("#admin-user-crypto-receive-confirm-btn-text").text();
            ...
            form.submit();
            setTimeout(function(){
                $("#admin-user-crypto-receive-confirm-btn-text").html(pretext + '<i class="fa fa-angle-right"></i>');
                $("#admin-user-crypto-receive-confirm-btn").removeAttr("disabled");
                $(".fa-spin").hide();
            },10000);
        }
    });
