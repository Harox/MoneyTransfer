  ...
  <div class="row">
      ...
      <div class="col-md-9">
          <div class="box box-info">
              ...
              <form action="{{ url('admin/save-preference') }}" method="post" id="preference-form" class="form-horizontal">
                {!! csrf_field() !!}
                <div class="box-body">
                  ...
                  <!-- Enable Currencies -->
                  <div class="form-group enabled-currencies-div">
                      <label class="col-sm-4 control-label" for="enable_currencies">Enabled Currencies</label>
                      <div class="col-sm-6">
                        <select name="enable_currencies" class="select2" id="enable_currencies">
                            <option value="fiat" {{isset($prefData['preference']['enable_currencies']) && $prefData['preference']['enable_currencies'] == 'fiat' ? 'selected':""}}>Fiat</option>
                            <option value="fiat_and_crypto" {{isset($prefData['preference']['enable_currencies']) && $prefData['preference']['enable_currencies'] == 'fiat_and_crypto' ? 'selected':""}}>Fiat & Crypto</option>
                        </select>
                      </div>
                  </div>
                  ...
                  @if(Common::has_permission(\Auth::guard('admin')->user()->id, 'edit_preference'))
                    <div class="row">
                        <div class="col-md-10">
                          <button type="submit" class="btn btn-primary btn-flat pull-right" id="preference-submit">
...
@push('extra_body_scripts')
  ...
  <script type="text/javascript">
    ...
    $(window).on('load',function()
    {
        $(".select2").select2();
        //
        var checkCryptoCurrencyPref = window.localStorage.getItem("crypto-disabled");
        if (JSON.parse(checkCryptoCurrencyPref) == true) // or JSON.parse(checkCryptoCurrencyPref) // local storage always sets string as value, hence, parsed for boolean check
        {
            $('#enable_currencies').select2('open');
            window.localStorage.removeItem("crypto-disabled");
        }
        //
    });
